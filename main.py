...
def hash_encrypt(input_array):
    array = []
    for ele in input_array:
        x = ord(ele)
        val = (inv(2)*x)%M
        array.append(chr(val))

    return array


def hash_decrypt(input_array):
    array = []
    for ele in input_array:
        x = ord(ele)
        val = (2*x)%M
...
def encode():
    ...
    run_length_encoded_data = run_length_encode(data_for_run_length_encode)
    encoded_data = hash_encrypt(run_length_encoded_data)
...
def decode():
    ...
    code = file_read(u)
    decoded_result = get_text_array(code,hash_map)
    result = hash_decrypt(decoded_result)
